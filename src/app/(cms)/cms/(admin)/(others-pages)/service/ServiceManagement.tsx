...
import { CreateServiceRequest, UpdateServiceRequest, Service } from "@/app/types";
...
interface LandingPageForm {
  ...
  hero: HeroForm; about: AboutForm; highlights: HighlightForm[];
  pricing: PricingForm; aboutCompany: AboutCompanyForm; clientStats: ClientStatsForm;
  solutions: SolutionsForm; projects: ProjectsForm; testimonials: TestimonialsForm;
  awards: AwardsForm; process: ProcessForm; team: TeamForm;
  ctaSection: CtaForm; qnA: QnAForm;
}
...
const initialFormData: ServiceFormData = {
  title: "", description: "", icon: "", image: "", isActive: true, displayOrder: 1, features: [],
};
...
function mapPageDTOtoLanding(page: ServicePageData): LandingPageForm {
  ...
  return {
    slug: s(page.slug), metaTitle: s(page.metaTitle), metaDescription: s(page.metaDescription),
    hero: page.hero ? {
      badge: s(page.hero.badge), title: s(page.hero.title), highlightText: s(page.hero.highlightText),
      subtitle: s(page.hero.subtitle), ctaText: s(page.hero.ctaText), ctaUrl: s(page.hero.ctaUrl),
      ctaSecondaryText: s(page.hero.ctaSecondaryText), ctaSecondaryUrl: s(page.hero.ctaSecondaryUrl),
    ...
    about: page.about ? {
      sectionTitle: s(page.about.sectionTitle), sectionSubtitle: s(page.about.sectionSubtitle),
      content: s(page.about.content), imageUrl: s(page.about.imageUrl), videoUrl: s(page.about.videoUrl),
      ctaText: s(page.about.ctaText), ctaUrl: s(page.about.ctaUrl),
      stats: (page.about.stats ?? []).map(st => ({ value: s(st.value), label: s(st.label), icon: s(st.icon) })),
    } : initialLandingPage.about,
    highlights: (page.highlights ?? []).map((h, i) => ({ icon: s(h.icon), title: s(h.title), description: s(h.description), displayOrder: i + 1, isActive: true })),
    pricing: page.pricing ? {
      title: s(page.pricing.title), subtitle: s(page.pricing.subtitle),
      packages: (page.pricing.packages ?? []).map((p, i) => ({
        name: s(p.name), price: s(p.price), priceNote: s(p.priceNote), isPopular: p.isPopular ?? false,
        ctaText: s(p.ctaText), ctaUrl: s(p.ctaUrl), displayOrder: i + 1,
        features: Array.isArray(p.features) ? p.features.map(f => String(f ?? "")) : [], isActive: true,
      })),
    } : initialLandingPage.pricing,
    aboutCompany: page.aboutCompany ? {
      sectionTitle: s(page.aboutCompany.sectionTitle), sectionSubtitle: s(page.aboutCompany.sectionSubtitle),
      content: s(page.aboutCompany.content), imageUrl: s(page.aboutCompany.imageUrl),
      ctaText: s(page.aboutCompany.ctaText), ctaUrl: s(page.aboutCompany.ctaUrl),
    ...
    clientStats: page.clientStats ? {
      sectionTitle: s(page.clientStats.sectionTitle), totalClients: s(page.clientStats.totalClients),
    ...
    solutions: page.solutions ? {
      title: s(page.solutions.title), subtitle: s(page.solutions.subtitle),
      items: (page.solutions.items ?? []).map((item, i) => ({
        icon: s(item.icon), title: s(item.title), description: s(item.description),
        imageUrl: s(item.imageUrl), ctaText: s(item.ctaText), ctaUrl: s(item.ctaUrl), displayOrder: i + 1, isActive: true,
      })),
    } : initialLandingPage.solutions,
    projects: page.projects ? {
      sectionTitle: s(page.projects.sectionTitle), sectionSubtitle: s(page.projects.sectionSubtitle),
      items: (page.projects.items ?? []).map((p, i) => ({
        title: s(p.title), client: s(p.client), description: s(p.description), thumbnailUrl: s(p.thumbnailUrl),
        resultText: s(p.resultText), ctaText: s(p.ctaText), ctaUrl: s(p.ctaUrl), displayOrder: i + 1, isActive: true,
      })),
    } : initialLandingPage.projects,
    testimonials: page.testimonials ? {
      title: s(page.testimonials.title), subtitle: s(page.testimonials.subtitle),
      items: (page.testimonials.items ?? []).map((t, i) => ({
        authorName: s(t.authorName), authorRole: s(t.authorRole), authorCompany: s(t.authorCompany),
        authorAvatarUrl: s(t.authorAvatarUrl), content: s(t.content), rating: n(t.rating, 5), displayOrder: i + 1, isActive: true,
      })),
    } : initialLandingPage.testimonials,
    awards: page.awards ? {
      title: s(page.awards.title), subtitle: s(page.awards.subtitle),
      items: (page.awards.items ?? []).map((a, i) => ({
        name: s(a.name), organization: s(a.organization), year: s(a.year), logoUrl: s(a.logoUrl), displayOrder: i + 1, isActive: true,
      })),
    } : initialLandingPage.awards,
    process: page.process ? {
      title: s(page.process.title), subtitle: s(page.process.subtitle),
    ...
    qnA: page.qnA ? {
      title: s(page.qnA.title), subtitle: s(page.qnA.subtitle),
...
function StatRows({ stats, onAdd, onUpdate, onRemove, placeholder }: {
  ...
  onAdd: () => void; onUpdate: (i: number, f: string, v: string) => void; onRemove: (i: number) => void; placeholder?: string;
}) {
  return (
...
export default function ServiceManagement() {
  ...
  const handleSelectRow = async (service: Service) => {
    ...
    setFormData({
      ...
      features: service.features.map(f => ({ id: f.id, content: f.content, displayOrder: f.displayOrder, isActive: f.isActive })),
    });
  ...
  const buildLandingPayload = () => {
    ...
    return {
      slug: lp.slug || undefined, metaTitle: lp.metaTitle || undefined, metaDescription: lp.metaDescription || undefined,
      hero: lp.hero.title ? {
        badge: lp.hero.badge || undefined, title: lp.hero.title, highlightText: lp.hero.highlightText || undefined,
        subtitle: lp.hero.subtitle || undefined, ctaText: lp.hero.ctaText || undefined, ctaUrl: lp.hero.ctaUrl || undefined,
      ...
      about: lp.about.content ? {
        ...
        content: lp.about.content, imageUrl: lp.about.imageUrl || undefined, videoUrl: lp.about.videoUrl || undefined,
        ctaText: lp.about.ctaText || undefined, ctaUrl: lp.about.ctaUrl || undefined,
    ...
    try {
      ...
      if (isEditing && selectedId) {
        await serviceApi.update({
          id: selectedId, title: formData.title, description: formData.description,
          icon: formData.icon || undefined, image: formData.image || undefined,
          isActive: formData.isActive, displayOrder: formData.displayOrder,
          features: formData.features, ...landingPayload,
            ...
            <div className="p-5">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                <div className="lg:col-span-2">
                  ...
                  <Input name="title" type="text" placeholder="Nhập tiêu đề service" value={formData.title} onChange={handleChange} error={!!errors.title} hint={errors.title || ""} />
                </div>
                {/* Icon */}
                <div>
                  <Label>Icon</Label>
                ...
                <div>
                  <Label>Thứ tự</Label>
                  <Input name="displayOrder" type="number" min="1" value={formData.displayOrder} onChange={handleChange} error={!!errors.displayOrder} hint={errors.displayOrder} />
                </div>
                {/* isActive */}
            ...
            <div className="p-5 space-y-4">
              ...
              <SectionCard title="SEO & Slug" icon={<Settings className="w-4 h-4 text-gray-400" />}>
                <div className="grid md:grid-cols-2 gap-4">
                  <FieldRow label="Slug (URL)">
                    <Input type="text" placeholder="seo-marketing..." value={lp.slug} onChange={e => setLP(p => ({ ...p, slug: e.target.value }))} />
                    <p className="mt-1 text-xs text-gray-400">dich-vu/<strong>{lp.slug || "..."}</strong></p>
                  </FieldRow>
              ...
              <SectionCard title="1. Hero Section" icon={<ImageIcon className="w-4 h-4 text-brand-500" />}>
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <FieldRow label="Hero Image">
                    <ImageUploadField value={lp.hero.heroImageUrl} onChange={v => lpSet("hero", { heroImageUrl: v })} folder="services/hero" previewSize="lg" />
                  </FieldRow>
              ...
              <SectionCard title="2. About Service — Giới thiệu dịch vụ" icon={<BookOpen className="w-4 h-4 text-indigo-500" />} defaultOpen={false}>
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <FieldRow label="Image" span={2}>
                    <ImageUploadField value={lp.about.imageUrl} onChange={v => lpSet("about", { imageUrl: v })} folder="services/about" previewSize="lg" />
                  </FieldRow>
                  ...
                  <FieldRow label="Nội dung" span={2}><TextArea value={lp.about.content} onChange={v => lpSet("about", { content: v })} placeholder="Mô tả chi tiết về dịch vụ..." rows={5} /></FieldRow>
                </div>
              ...
              <SectionCard title="3. Highlights — Điểm nổi bật" icon={<Zap className="w-4 h-4 text-yellow-500" />} defaultOpen={false}>
                ...
                <div className="space-y-3">
                  {lp.highlights.map((h, i) => (
                    <ItemCard key={i} index={i} total={lp.highlights.length}
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="space-y-4">
                    {lp.pricing.packages.map((pkg, pi) => (
                      <ItemCard key={pi} index={pi} total={lp.pricing.packages.length}
              ...
              <SectionCard title="5. About Company — Về công ty" icon={<Building2 className="w-4 h-4 text-blue-500" />} defaultOpen={false}>
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <FieldRow label="Image" span={2}>
                    <ImageUploadField value={lp.aboutCompany.imageUrl} onChange={v => lpSet("aboutCompany", { imageUrl: v })} folder="services/company" previewSize="lg" />
                  </FieldRow>
                  ...
                  <FieldRow label="Nội dung" span={2}><TextArea value={lp.aboutCompany.content} onChange={v => lpSet("aboutCompany", { content: v })} placeholder="Giới thiệu về công ty..." rows={4} /></FieldRow>
                </div>
                  ...
                  <div className="space-y-3">
                    {lp.clientStats.logos.map((logo, i) => (
                      <div key={i} className="p-3 bg-gray-50 dark:bg-gray-800/40 rounded-lg border border-gray-200 dark:border-gray-700 space-y-2">
                        ...
                        <ImageUploadField value={logo.logoUrl} onChange={v => updateLogo(i, "logoUrl", v)} folder="logos" previewSize="sm" maxSizeMB={1} />
                      </div>
                    ))}
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="space-y-3">
                    {lp.solutions.items.map((item, i) => (
                      <ItemCard key={i} index={i} total={lp.solutions.items.length}
                        onMoveUp={() => setLP(p => ({ ...p, solutions: { ...p.solutions, items: moveItem(p.solutions.items, i, "up") } }))}
                        onMoveDown={() => setLP(p => ({ ...p, solutions: { ...p.solutions, items: moveItem(p.solutions.items, i, "down") } }))}
                        onRemove={() => setLP(p => ({ ...p, solutions: { ...p.solutions, items: removeItem(p.solutions.items, i) } }))}>
                        <div className="grid md:grid-cols-2 gap-3">
                          ...
                          <FieldRow label="Image" span={2}>
                            <ImageUploadField value={item.imageUrl} onChange={v => updateSolution(i, "imageUrl", v)} folder="services/solutions" />
                          </FieldRow>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="space-y-3">
                    {lp.projects.items.map((p, i) => (
                      <ItemCard key={i} index={i} total={lp.projects.items.length}
                        onMoveUp={() => setLP(prev => ({ ...prev, projects: { ...prev.projects, items: moveItem(prev.projects.items, i, "up") } }))}
                        onMoveDown={() => setLP(prev => ({ ...prev, projects: { ...prev.projects, items: moveItem(prev.projects.items, i, "down") } }))}
                        onRemove={() => setLP(prev => ({ ...prev, projects: { ...prev.projects, items: removeItem(prev.projects.items, i) } }))}>
                        <div className="grid md:grid-cols-2 gap-3">
                          ...
                          <FieldRow label="Thumbnail">
                            <ImageUploadField value={p.thumbnailUrl} onChange={v => updateProject(i, "thumbnailUrl", v)} folder="projects" previewSize="lg" />
                          </FieldRow>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="space-y-3">
                    {lp.testimonials.items.map((t, i) => (
                      <ItemCard key={i} index={i} total={lp.testimonials.items.length}
                        onMoveUp={() => setLP(p => ({ ...p, testimonials: { ...p.testimonials, items: moveItem(p.testimonials.items, i, "up") } }))}
                        onMoveDown={() => setLP(p => ({ ...p, testimonials: { ...p.testimonials, items: moveItem(p.testimonials.items, i, "down") } }))}
                        ...
                        <div className="grid md:grid-cols-2 gap-3">
                          ...
                          <FieldRow label="Avatar">
                            <ImageUploadField value={t.authorAvatarUrl} onChange={v => updateTestimonial(i, "authorAvatarUrl", v)} folder="avatars" previewSize="sm" maxSizeMB={1} />
                          </FieldRow>
                ...
                <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="space-y-3">
                    {lp.awards.items.map((award, i) => (
                      <ItemCard key={i} index={i} total={lp.awards.items.length}
                        ...
                        <div className="grid md:grid-cols-2 gap-3">
                          ...
                          <FieldRow label="Logo">
                            <ImageUploadField value={award.logoUrl} onChange={v => updateAward(i, "logoUrl", v)} folder="awards" previewSize="sm" maxSizeMB={1} />
                          </FieldRow>
                        </div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="space-y-3">
                    {lp.process.steps.map((step, i) => (
                      <ItemCard key={i} index={i} total={lp.process.steps.length}
              ...
              <SectionCard title="12. Team — Đội ngũ" icon={<Users className="w-4 h-4 text-purple-500" />} defaultOpen={false}>
                ...
                {resolvedExperts.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                    <p className="text-xs font-medium text-gray-500 mb-2">👥 Thành viên hiện tại</p>
                  ...
                  <div className="space-y-2">
                    {lp.team.memberTeamIds.map((mid, i) => (
                      <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800/40 rounded-lg border border-gray-200 dark:border-gray-700">
                        ...
                        <Input type="number" min="1" placeholder="ID thành viên" value={mid || ""} onChange={e => updateTeamMember(i, parseInt(e.target.value) || 0)} />
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="space-y-3">
                    {lp.qnA.items.map((q, i) => (
                      <ItemCard key={i} index={i} total={lp.qnA.items.length}
                        onMoveUp={() => setLP(p => ({ ...p, qnA: { ...p.qnA, items: moveItem(p.qnA.items, i, "up") } }))}
                        onMoveDown={() => setLP(p => ({ ...p, qnA: { ...p.qnA, items: moveItem(p.qnA.items, i, "down") } }))}
                        onRemove={() => setLP(p => ({ ...p, qnA: { ...p.qnA, items: removeItem(p.qnA.items, i) } }))}>
                        ...
                        <div className="flex items-center gap-2 pt-1"><input type="checkbox" checked={q.isActive} onChange={e => updateQnA(i, "isActive", e.target.checked)} className="w-4 h-4 rounded" /><span className="text-sm text-gray-600 dark:text-gray-400">Hiển thị</span></div>
                      </ItemCard>
                    ))}
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/3">
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Tổng: {filteredServices.length}</p>
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                {["Chọn", "Ảnh", "Service", "Icon", "Slug", "Tính năng", "Thứ tự", "Trạng thái", "Xóa"].map(h => (
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
              {loading && services.length === 0 ? (
                <tr><td colSpan={9} className="px-5 py-10 text-center"><Loader2 className="w-8 h-8 animate-spin text-brand-500 mx-auto" /></td></tr>
              ) : filteredServices.length === 0 ? (
                <tr><td colSpan={9} className="px-5 py-10 text-center"><p className="text-gray-500">{searchTerm ? "Không tìm thấy" : "Chưa có service nào"}</p></td></tr>
                ...
                <tr key={service.id} className={`transition-colors hover:bg-gray-50 dark:hover:bg-white/2 ${selectedId === service.id ? "bg-brand-50 dark:bg-brand-500/10" : ""}`}>
                  ...
                  {/* ✅ Cột ảnh mới */}
<td className="px-4 py-4 text-center">
  {service.image ? (  // ✅ đổi service.image → service.image
    <img src={service.image} alt={service.title} className="w-14 h-10 object-cover rounded-lg border border-gray-200 dark:border-gray-700 mx-auto" />
  ) : (
    <div className="w-14 h-10 rounded-lg border-2 border-dashed border-gray-200 dark:border-gray-700 flex items-center justify-center mx-auto">
      <ImageIcon className="w-4 h-4 text-gray-300" />
                  ...
                  <td className="px-4 py-4">
                    ...
                    <p className="text-xs text-gray-500 truncate max-w-64">{service.description}</p>
                  </td>
                  <td className="px-4 py-4 text-center">
                    {service.icon ? <span className="inline-flex items-center justify-center w-10 h-10 bg-brand-50 text-brand-600 rounded-lg dark:bg-brand-500/10"><DynamicIcon name={service.icon} className="w-5 h-5" /></span> : <span className="text-gray-400">-</span>}
                  </td>
                  <td className="px-4 py-4 text-center">
                    {service.slug ? <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-50 text-purple-700 dark:bg-purple-500/10 dark:text-purple-400">/{service.slug}</span> : <span className="text-gray-400 text-xs">Chưa có</span>}
                  </td>
                  <td className="px-4 py-4 text-center">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700 dark:bg-blue-500/10">{service.features.length} tính năng</span>
                  </td>
                  <td className="px-4 py-4 text-center"><span className="text-sm text-gray-600">{service.displayOrder}</span></td>
                  <td className="px-4 py-4 text-center">
                    {service.isActive
                      ? <span className="inline-flex items-center gap-1 text-success-600"><CheckCircleIcon className="w-5 h-5" /><span className="text-xs">Hiện</span></span>
                      : <span className="inline-flex items-center gap-1 text-gray-400"><XIcon className="w-5 h-5" /><span className="text-xs">Ẩn</span></span>}
                  </td>
                  <td className="px-4 py-4 text-center">
                    <button onClick={() => handleDelete(service.id)} disabled={loading} className="p-2 text-error-500 hover:bg-error-50 rounded-lg disabled:opacity-50"><TrashBinIcon className="w-5 h-5" /></button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="px-5 py-4 border-t border-gray-200 dark:border-gray-800">
          <p className="text-sm text-gray-500">Hiển thị {filteredServices.length} service</p>
        </div>
      </div>
    </div>
  );

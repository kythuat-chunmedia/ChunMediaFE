...
import { uploadApi } from "@/app/lib/api/endpoints/upload.api";
import { PlusIcon, PencilIcon, TrashBinIcon, CheckCircleIcon, XIcon, EyeIcon } from "./icons";
import { TiptapEditor, TiptapEditorRef } from "@/app/components/tiptap-editor"; // ✅ import TiptapEditor
...
// ============ COMPONENT ============
export default function PortfolioManagement() {
  const editorRef = useRef<TiptapEditorRef>(null); // ✅ ref cho TiptapEditor
  ...
  // ✅ Handler riêng cho TiptapEditor — KHÔNG fake event, tránh infinite loop
  ...
  // ✅ Validate — dùng editorRef.current?.isEmpty thay vì chỉ check formData
  const validate = (): boolean => {
    const newErrors: Record<string, string> = {};
    ...
    // ✅ Validate content từ editor ref
    ...
    // }
  const latestHtml = editorRef.current?.getHtml() || formData.content || "";
  if (!latestHtml.trim() || latestHtml === "<p></p>") {
    newErrors.content = "Vui lòng nhập nội dung chi tiết";
  }
  ...
  // ✅ Select row — set content qua ref, KHÔNG qua prop (tránh infinite loop)
  const handleSelectRow = (portfolio: Portfolio) => {
    setSelectedId(portfolio.id);
    setIsEditing(true);
    setFormData({
    ...
    // ✅ Set editor content qua ref
    setTimeout(() => {
      editorRef.current?.setContent(portfolio.content || "");
    }, 0);

    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  // ✅ Reset — clear editor qua ref
  const handleReset = () => {
    setSelectedId(null);
    setIsEditing(false);
    setFormData(initialFormData);
  ...
  // ✅ Submit — lấy content mới nhất từ editor ref trước khi validate/submit
  const handleSubmit = async (e: React.FormEvent) => {
    console.log("start");
    e.preventDefault();
    ...
    try {
      setLoading(true);
      const submitData = { ...formData, content: latestHtml };
      ...
      if (isEditing && selectedId) {
        ...
        const portfolioToUpdate: Portfolio = {
          id: selectedId,
          ...submitData,
          seoTitle: submitData.seoTitle || null,
          seoDescription: submitData.seoDescription || null,
          shortDescription: submitData.shortDescription || null,
          bannerUrl: submitData.bannerUrl || null,
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold text-gray-800 dark:text-white/90">
                {formatNumber(kpiSummary.totalReach)}
              </p>
              <p className="text-xs text-gray-500">Reach</p>
            </div>
          </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold text-gray-800 dark:text-white/90">
                {formatNumber(kpiSummary.totalEngagement)}
              </p>
              <p className="text-xs text-gray-500">Engagement</p>
            </div>
          </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold text-gray-800 dark:text-white/90">
                {kpiSummary.avgConversionRate.toFixed(1)}%
              </p>
              <p className="text-xs text-gray-500">Avg. CVR</p>
            </div>
          </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold text-gray-800 dark:text-white/90">
                {formatNumber(kpiSummary.totalOrderQuantity)}
              </p>
              <p className="text-xs text-gray-500">Đơn hàng</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold text-brand-700 dark:text-brand-400">
                {formatNumber(kpiSummary.totalRevenue)}
              </p>
              <p className="text-xs text-brand-600 dark:text-brand-500">Doanh thu</p>
            </div>
          </div>
        ...
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-800">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {isEditing
              ...
              : "Điền thông tin để thêm case study mới"}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5">
          ...
          {activeTab === "basic" && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              ...
              <div>
                <Label>
                  Năm thực hiện <span className="text-error-500">*</span>
                </Label>
                <Input
                  name="year"
                  type="number"
              ...
              <div className="lg:col-span-2">
                <Label>
                  Ảnh Thumbnail <span className="text-error-500">*</span>
                </Label>
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    <input
                    ...
                    {errors.thumbnailUrl && (
                      <p className="mt-1.5 text-xs text-error-500">{errors.thumbnailUrl}</p>
                    )}
                    {!errors.thumbnailUrl && !uploadingThumbnail && (
                      <p className="mt-1.5 text-xs text-gray-500">
                        Ảnh đại diện cho dự án (Tối đa 2MB)
                      </p>
                    )}
                  </div>
                  {formData.thumbnailUrl && (
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    ...
                    {!uploadingBanner && (
                      <p className="mt-1.5 text-xs text-gray-500">Ảnh header lớn (optional)</p>
                    )}
                  </div>
                  {formData.bannerUrl && (
                    <div className="relative">
          ...
          {/* ✅ TAB: Content — thay textarea bằng TiptapEditor */}
          {activeTab === "content" && (
            <div className="space-y-5">
              {/* Short Description — giữ nguyên textarea */}
              <div>
                <Label>Mô tả ngắn</Label>
                <textarea
                  name="shortDescription"
              ...
              {/* ✅ Nội dung chi tiết — TiptapEditor thay textarea */}
              <div>
                <Label>
                  Nội dung chi tiết <span className="text-error-500">*</span>
                </Label>
                <div
                  className={`rounded-lg ${errors.content ? "ring-2 ring-error-500/30" : ""}`}
                >
                  <TiptapEditor
                    ref={editorRef}
                    content={formData.content || ""}
                    onChange={handleContentChange}
                    placeholder="Nhập nội dung chi tiết: bối cảnh, thách thức, giải pháp, kết quả..."
                    showWordCount
                ...
                {errors.content ? (
                  ...
                  <p className="mt-1.5 text-xs text-gray-500">
                    Nội dung HTML chi tiết của case study (hỗ trợ kéo thả hình ảnh, resize ảnh)
                  </p>
                )}
              </div>
            </div>
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    {formData.seoDescription ||
                      ...
                      "Mô tả ngắn về dự án sẽ hiển thị ở đây..."}
                  </p>
                </div>
              </div>
            </div>

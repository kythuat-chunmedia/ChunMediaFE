...
import {
  WEBSITE_TYPE_OPTIONS, BUDGET_UI_OPTIONS, TIMELINE_UI_OPTIONS, FEATURE_UI_OPTIONS,
  DESIGN_STYLE_OPTIONS, ADDON_UI_OPTIONS, HAS_LOGO_OPTIONS, HAS_WEBSITE_OPTIONS,
  CONTENT_READY_OPTIONS, HAS_DOMAIN_OPTIONS,
} from "@/app/types";
...
function findLabel<T extends { value: number; label?: string; title?: string }>(options: T[], value: number | null): string {
  if (value === null) return "";
  const opt = options.find((o) => o.value === value);
  return opt ? (opt as any).title || (opt as any).label || "" : "";
}

function findLabels<T extends { value: number; label?: string; title?: string }>(options: T[], values: number[]): string {
  return values.map((v) => findLabel(options, v)).filter(Boolean).join(", ");
}

interface ReviewItem { label: string; value: string; full?: boolean; colorPreview?: boolean; }
...
export default function StepReview({ formData, errors, updateField }: Props) {
  ...
  const sections: { title: string; emoji: string; items: ReviewItem[] }[] = [
    {
      title: "Thông tin liên hệ", emoji: "👤",
      items: [
    ...
    },
    {
      ...
      items: [
        ...
        ...(formData.features.length ? [{ label: "Tính năng", value: findLabels(FEATURE_UI_OPTIONS, formData.features), full: true }] : []),
      ],
    },
    {
      title: "Thiết kế", emoji: "🎨",
      items: [
        ...
        ...(formData.designStyles.length ? [{ label: "Phong cách", value: findLabels(DESIGN_STYLE_OPTIONS, formData.designStyles), full: true }] : []),
      ],
    },
    {
      title: "Ngân sách", emoji: "💰",
      items: [
        ...
        ...(formData.addons.length ? [{ label: "Dịch vụ thêm", value: findLabels(ADDON_UI_OPTIONS, formData.addons), full: true }] : []),
      ],
    },
  ];
  ...
  return (
    <div>
      <StepHeader icon="✅" iconBg="rgba(16,185,129,0.12)" iconColor="#10b981"
      ...
      {sections.map((sec) => {
        ...
        return (
          <div key={sec.title} className="mb-6">
            <h3 className="text-sm font-extrabold text-[#0A9396] mb-4 flex items-center gap-2">
              <span>{sec.emoji}</span> {sec.title}
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {validItems.map((item) => (
                <div key={item.label}
                  className={`p-3.5 bg-[rgba(10,147,150,0.04)] border border-[rgba(10,147,150,0.12)] rounded-xl ${item.full ? "md:col-span-2" : ""}`}>
                  <div className="text-[11px] text-[#95A5A6] font-bold uppercase tracking-wider mb-1">{item.label}</div>
                  <div className="text-sm font-semibold text-[#1A1A1A] flex items-center gap-1.5">
                    {item.colorPreview && item.value && (
                      <span className="inline-block w-4 h-4 rounded-md border border-black/10" style={{ background: item.value }} />
                    )}
                    {item.value}
                  </div>
                </div>
      ...
      {/* Agreement */}
      <div className="mt-7 p-5 bg-[rgba(10,147,150,0.05)] border border-[rgba(10,147,150,0.2)] rounded-xl">
        <div className="flex items-start gap-3 cursor-pointer" onClick={() => updateField("agree", !formData.agree)}>
          <div className={`w-5 h-5 border-2 rounded-md shrink-0 flex items-center justify-center transition-all duration-200 mt-0.5 ${
            formData.agree
              ? "border-[#0A9396] bg-gradient-to-br from-[#0A9396] to-[#94D2BD]"
              : errors.agree ? "border-red-400" : "border-[rgba(10,147,150,0.35)]"
          }`}>
            {formData.agree && <span className="text-white text-[10px] font-black">✓</span>}
          </div>
          <p className="text-[13px] font-semibold leading-relaxed text-[#1A1A1A]">

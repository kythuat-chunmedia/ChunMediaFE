...
export default function StepFeatures({ formData, errors, updateField, toggleNumArray }: Props) {
  return (
    <div>
      ...
      <label className="block text-[13px] font-bold text-[#2C3E50] mb-3">Tính năng cần có</label>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-7">
        {FEATURE_UI_OPTIONS.map((f) => (
          <OptionCard key={f.value} selected={formData.features.includes(f.value)}
            onClick={() => toggleNumArray("features", f.value)} title={f.label} type="checkbox" />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <FormGroup label="Số lượng trang dự kiến">
          <SelectInput value={formData.pageCount} onChange={(v) => updateField("pageCount", v)}
        ...
        <FormGroup label="Ngôn ngữ website" required error={errors.language}>
          <SelectInput value={formData.language} onChange={(v) => updateField("language", v)}
        ...
        <div className="md:col-span-2">
          <FormGroup label="Yêu cầu đặc biệt" optional>
            <TextArea value={formData.specialRequirements} onChange={(v) => updateField("specialRequirements", v)}
              placeholder="VD: Tích hợp phần mềm, API bên thứ 3, hệ thống điểm thưởng..." rows={3} />
          </FormGroup>
        </div>
      </div>
    </div>
